...
def read_issue_cards(html):
    ...
    for card in cards:
        ...
        issues.append({
            ...
            "detail_url": detail_url,       # kept — exported in JSON
            "price_low": price_low,
            "price_mid": price_mid,
            ...
            "variant_url": variant_url,     # kept — exported in JSON
            "variants": [],
        })
    return issues


async def scrape_series(session, series_url, log_q, series_title):
    html = await fetch(session, series_url)
    soup = BeautifulSoup(html, "html.parser")
    total_pages = get_total_pages(soup)
    log_q.put(("info", f"  Pages to scan: {total_pages}"))
    ...
    async def scrape_issue_page(page_url, page_num):
        log_q.put(("info", f"    Reading page {page_num}/{total_pages}…"))
        html = await fetch(session, page_url)
        issues = read_issue_cards(html)

        variant_tasks = [
            scrape_variants(idx, issue["variant_url"], issue["title"])
        ...
        for result in variant_results:
            if isinstance(result, Exception):
                log_q.put(("error", "    Variant fetch failed"))
                continue
            idx, variant_issues = result
            issues[idx]["variants"] = variant_issues

        log_q.put(("ok", f"    Page {page_num} done — {len(issues)} issues found"))
        return issues

    async def scrape_variants(idx, variant_url, issue_title):
        log_q.put(("warn", f"    → Fetching variants for: {issue_title}"))
        variant_html = await fetch(session, variant_url)
        return idx, read_issue_cards(variant_html)

    results = await asyncio.gather(
        *[scrape_issue_page(u, i + 1) for i, u in enumerate(page_urls)],
        return_exceptions=True,
    )
    all_issues = []
    for result in results:
        if isinstance(result, Exception):
            log_q.put(("error", "  Issue page failed to load"))
            continue
    ...
    async with aiohttp.ClientSession(headers=headers, timeout=timeout, connector=connector) as session:
        ...
        async def scrape_search_page(page_url, sp):
            ...
            async def scrape_one_series(series_data, index):
                log_q.put(("info", f"  [{index + 1}/{len(series_list)}] {series_data['title']} ({series_data['total_issues']} issues)"))
                if series_data["total_issues"] > 0:
                    series_data["issues"] = await scrape_series(
                        session, series_data["url"], log_q, series_data["title"]
                    )
                return series_data

            results = await asyncio.gather(
                *[scrape_one_series(s, i) for i, s in enumerate(series_list)],
                return_exceptions=True,
            )
            clean = []
            for result in results:
                if isinstance(result, Exception):
                    log_q.put(("error", "  Series failed to load"))
                    continue
                clean.append(result)
            return clean

        search_results = await asyncio.gather(
            *[scrape_search_page(u, i + 1) for i, u in enumerate(search_page_urls)],
            return_exceptions=True,
        )
        for result in search_results:
            if isinstance(result, Exception):
                log_q.put(("error", "Search page failed to load"))
                continue
...
with st.form("search_form"):
    ...
    with col_input:
        query = st.text_input(
            "Search query",
            placeholder="Spider-Man, Batman, X-Men…",
...
if run and query.strip():
    ...
    thread = threading.Thread(
        ...
        args=(query.strip(), log_q, result_q),
        daemon=True,
    )
    thread.start()
...
if st.session_state.results:
    ...
    # All URL fields (url, detail_url, variant_url) are preserved in the JSON export
    st.download_button(
        label="⬇ DOWNLOAD JSON",
    ...
    for series in data:
        ...
        with st.expander(label):
            if not issues:
                st.markdown(
                    '<span style="color:var(--muted,#888);font-size:0.8rem">No issues scraped.</span>',
                    unsafe_allow_html=True,
                )
                continue
                ...
                with c1:
                    ...
                    if iss.get("key_notes"):
                        for note in iss["key_notes"]:
                            st.markdown(
                                f'<span style="color:var(--muted,#888);font-size:0.72rem">• {note}</span>',
                                unsafe_allow_html=True,
                            )
                    if iss.get("tags"):
                        pills = "".join(f'<span class="tag-pill">{t}</span>' for t in iss["tags"])
                        st.markdown(pills, unsafe_allow_html=True)
                    if iss.get("has_variants"):
                        st.markdown(
                            f'<span style="color:var(--muted,#6b6b80);font-size:0.7rem;'
                            f'font-family:\'IBM Plex Mono\',monospace">{iss["variant_count"]} variants</span>',
                            unsafe_allow_html=True,
                        )
                c2.markdown(iss.get("year") or "—")
